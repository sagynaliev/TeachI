...
const generateToken = (userId) => {
  const secret = process.env.JWT_SECRET || 'fallback-secret-for-development-mode';
...
router.post('/login', [
  ...
  try {
    console.log('🔐 BACKEND: Login request received', req.body); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
    
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      console.log('❌ BACKEND: Validation errors', errors.array()); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
      return res.status(400).json({ 
        message: 'Validation errors', 
        errors: errors.array() 
      });
    ...
    console.log('🔍 BACKEND: Looking for user with email:', email); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ

    // Find user by email
    const user = await User.findOne({ email });
    console.log('👤 BACKEND: User found:', user ? 'Yes' : 'No'); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
    
    if (!user) {
      console.log('❌ BACKEND: User not found'); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
      return res.status(400).json({ message: 'Invalid credentials' });
    }

    // Check if account is active
    if (!user.isActive) {
      console.log('❌ BACKEND: Account is deactivated'); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
      return res.status(400).json({ message: 'Account is deactivated' });
    }

    console.log('🔑 BACKEND: Comparing passwords...'); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
    // Verify password
    const isMatch = await user.comparePassword(password);
    console.log('🔑 BACKEND: Password match:', isMatch); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
    
    if (!isMatch) {
      console.log('❌ BACKEND: Password does not match'); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
      return res.status(400).json({ message: 'Invalid credentials' });
    }
    ...
    console.log('✅ BACKEND: Login successful for user:', user.email); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
    ...
    console.error('💥 BACKEND: Login error:', error); // БҰЛ ЖОЛДЫ ҚОСЫҢЫЗ
